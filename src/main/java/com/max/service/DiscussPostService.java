...
public class DiscussPostService {
    ...
    public List<DiscussPost> findDiscussPosts(int userId, int setoff, int limit) {
        return discussPostMapper.selectDiscussPosts(userId, setoff, limit);
    }
     ...
     * 2、在得到每一个 DiscussPost 数据后，单独调用方法去查询
     * 选方法 2 ，耦合度低，可以改进的空间大
     */

    public int findDiscussRows(int userId) {
        //return findDiscussRows(userId);  //这里重大错误。导致了栈溢出
